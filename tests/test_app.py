...
def local_config_runtime(config_path: Path) -> CloudflaredRuntime:
...
def test_dashboard_without_configured_path_renders_safe_empty_state() -> None:
    ...
    assert "Config adoption" in response.text
    assert "Config load" in response.text
    ...
    assert "No configuration adopted" in response.text
    assert "Discovery disabled" in response.text
...
def test_dashboard_renders_detected_routes_from_explicit_fixture() -> None:
    ...
    assert "Adopted · Unverified" in response.text
...
def test_dashboard_shows_detected_config_without_loading_or_disclosing_path() -> None:
    ...
    response = get_from_app(
        create_app(
            settings,
            runtime_discovery=lambda enabled: local_config_runtime(detected_path),
        ),
        "/",
    )

    assert response.status_code == 200
    assert "Detected · Not adopted" in response.text
    assert "sudo cfm-config cloudflared-config adopt-detected" in response.text
    assert "No configuration adopted" in response.text
    assert str(detected_path) not in response.text
    assert "detected-config.yml" not in response.text


def test_dashboard_shows_adopted_config_matching_runtime_without_path() -> None:
    settings = Settings(
        mode="test",
        runtime_discovery_enabled=True,
        cloudflared_config_path=FIXTURE_PATH,
    )

    response = get_from_app(
        create_app(
            settings,
            runtime_discovery=lambda enabled: local_config_runtime(FIXTURE_PATH),
        ),
        "/",
    )

    assert response.status_code == 200
    assert "Adopted" in response.text
    assert "matches the config referenced by cloudflared.service" in response.text
    assert "dashboard.example.com" in response.text
    assert str(FIXTURE_PATH) not in response.text


def test_dashboard_shows_config_drift_without_disclosing_either_path() -> None:
    detected_path = Path("/private/cloudflared/runtime-config.yml")
    settings = Settings(
        mode="test",
        runtime_discovery_enabled=True,
        cloudflared_config_path=FIXTURE_PATH,
    ...
    assert response.status_code == 200
    assert "Adopted · Service differs" in response.text
    assert "No change was made automatically" in response.text
    assert "dashboard.example.com" in response.text
    assert str(FIXTURE_PATH) not in response.text
    assert str(detected_path) not in response.text
    assert "runtime-config.yml" not in response.text
...
def test_dashboard_survives_invalid_explicit_config_without_path_disclosure(
    ...
    assert "Adopted · Load error" in response.text
...
def test_dashboard_renders_sanitized_token_managed_runtime(
    ...
    assert "Token-managed" in response.text
    assert "no local config candidate to adopt" in response.text
    assert "Not configured" in response.text
    assert "No configuration adopted" in response.text
...
def test_dashboard_survives_runtime_discovery_error_without_disclosure() -> None:
    ...
    assert "Discovery unavailable" in response.text
    assert FAKE_SECRET not in response.text
    assert "Traceback" not in response.text


def test_dashboard_keeps_adopted_routes_when_runtime_discovery_fails() -> None:
    def failed_discovery(enabled: bool):
        raise CommandTimedOutError(FAKE_SECRET)
