...
| After `SERVICE_VERIFIED` but before durable cleanup-decision publication | Candidate/service startup was already durably verified; staging may contain a proposed cleanup decision | Do not issue another service command or select automatic rollback. Discard interrupted staging, reauthenticate candidate-active filesystem and the current supported service shape, and publish `COMMIT_CLEANUP_PENDING` only if verification is available; otherwise retain `SERVICE_VERIFIED` and return recovery required |
